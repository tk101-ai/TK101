...
- [ ] **문서작업 Phase3b — 양식 채우기 위저드 재구성**(남은 큰 UX 작업):
  - 5페이지 강제이동 → 단일 화면 + antd `Steps` 위저드(업로드+변수확인 병합, 페이지 네비 제거)
  - 실제 docx 미리보기(현재 가짜 텍스트덤프) · "확정 후 다운로드" 3중동작 단순화 · 작성중 잡 목록(resume)
  - DocGenPage(단일화면·인라인편집·일괄재생성) UX를 템플릿 삼아 정렬

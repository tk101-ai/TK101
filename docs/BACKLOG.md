...
- [ ] **문서작업 Phase3b-2 — 양식 위저드 심화**(남은 큰 작업, 선택):
  - 단일 화면 단일컴포넌트 병합(페이지 reload 제거 — 현재 상태는 서버영속이라 데이터손실은 없음, 체감개선 목적)
  - **실제 docx 미리보기**(현재 가짜 텍스트덤프 → mammoth 등 docx→HTML 렌더 라이브러리 도입 필요)
  - "확정 후 다운로드" 3중동작 단순화 · 작성중 잡 목록(resume, `getJobRevisions` 미사용 UI 활용)
...
- [ ] **P2: Qdrant `text`/`path` 풀텍스트 payload 인덱스 생성**(현재 없음 확인) — 키워드 arm이 4000-scroll(코퍼스 0.6%)에 갇힘. MatchText로 전환 시 정확매칭+레이턴시 동시 개선. 재임베딩 불필요.

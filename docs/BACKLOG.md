...
> - **남은 Phase3 = 양식 채우기 UX**(아래 NEXT): 5페이지→위저드 / 진행률·에러표시 / 스마트기본값·필터 / 텔레메트리 숨김 / 실제 미리보기
...
**내일 정하면 바로 진행할 후보:**
- [ ] ⭐**1순위 · NAS 풀텍스트 인덱스(P2)** — 가성비 최고. **반나절·비용0·무재임베딩·무중단.** 키워드(정확검색) arm이 지금 코퍼스 0.6%만 스캔해 품번·코드·고유명사·파일명을 거의 못 잡음 → Qdrant `text`/`path` 풀텍스트 payload 인덱스 + MatchText 전환으로 **정확매칭을 전체 코퍼스로 복원**. 임베딩(의미검색)은 그대로 주력으로 두고 둘을 RRF로 합침(대체 아님, 보완). 파일명 인덱싱으로 H4(파일명 미임베딩)도 재임베딩 없이 상당부분 메움. 키워드 arm은 스캔→색인조회라 오히려 빨라짐. 상세=`NAS 검색 품질 후속`/메모리 [[nas-search-tuning]].
...
- [ ] ⭐**P2: Qdrant `text`/`path` 풀텍스트 payload 인덱스 생성**(현재 없음 확인) → **NOW 1순위로 승격**. 키워드 arm이 4000-scroll(코퍼스 0.6%)에 갇힘. MatchText로 전환 시 정확매칭(품번·코드·고유명사·파일명)을 전체 코퍼스로 복원 + 레이턴시 개선. 임베딩과 RRF로 병용(대체 아님). 재임베딩·비용 불필요, 반나절·무중단. 진행 시 전/후 정확매칭+속도 측정.
- [ ] **H3: 엑셀 행에 헤더 prepend / 문서 구조 보존** — `12 | 서울 | 3500`처럼 헤더 없는 데이터행은 임베딩 의미 약함.
- [ ] (선택) 검색 레이턴시 ~9~10s — 리랭크 풀20(+3s)+키워드 4000-scroll. P2(풀텍스트인덱스) 적용 시 scroll 제거로 단축. 필요시 top_n 16으로 절충 가능.

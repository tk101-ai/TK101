...
## NEXT (조만간)
- [ ] **문서작업 Phase3 — 양식 채우기 UX 개선**(품질 백엔드는 Phase1·2로 완료):
  - 5페이지 강제이동 → 단일 화면 + antd `Steps` 위저드(업로드+변수확인 병합)
  - 긴 LLM 작업 진행률(전체 오버레이 Spin+예상시간, "n/N 처리중") · 에러 원인표시(`err.response.data.detail`)
  - 자동 소스검색을 주 CTA로·자동실행 · "누락/확인필요만 보기" 필터(50행 테이블)
  - 내부 텔레메트리(Langfuse/토큰/USD)·"v0.1/Haiku" 노출 admin 게이트 · 실제 docx 미리보기
  - 양식 채우기는 DocGenPage(단일화면·인라인편집·일괄재생성) UX를 템플릿 삼아 정렬

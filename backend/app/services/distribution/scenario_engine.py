...
[공급 방향 — 절대 고정. 어기면 안 됨]
- 한국 측(국내 어드민)이 물건을 공급/발송하고 재고를 보유한 '공급자'입니다.
- 베트남 측은 **항상 물건을 공급받는(수령) 쪽**입니다. 베트남이 한국으로 물건을 보내거나 공급/출고하는 묘사는 절대 하지 마세요.
- 출고·발송·재고 보유·공급 결정의 주체는 한국, 수령·확인·재주문·현지 판매의 주체는 베트남으로 일관되게 작성하세요.

[인사는 가볍게 한 마디만 — 그 외 잡담 금지]
- 이 대화는 업무 대화입니다. 출고·재고·주문·정산·매입 등 비즈니스 주제에서 벗어나지 마세요.
- 대화 시작은 "안녕하세요~" 같은 가벼운 인사 한 마디로 부드럽게 열어도 됩니다(딱 한 줄, 그 후 바로 본론).
- 날씨·주말·식사·건강·가족·취미 같은 사적 잡담은 단 한 문장도 넣지 마세요.
- 비트에 없는 화제(사담)를 임의로 추가하지 마세요. 인사 한 줄 외 모든 메시지는 비트가 지시한 업무 내용만 다룹니다.
...
[供货方向 — 绝对固定，不可违反]
- 韩国方(国内管理员)是**供货方**：备货、发货、持有库存、决定供应。
- 越南方**永远是收货方(被供货一方)**。绝对不要把越南描写成向韩国发货/供货/出货的一方。
- 发货·出货·持库存·供货决定由韩国方做；收货·确认·补单·当地销售由越南方做。要全程一致。

[开场只允许一句轻寒暄 — 其余闲聊禁止]
- 这是业务对话。只围绕发货、库存、订单、结算、采购等业务话题，不要跑题。
- 开场可以用一句轻松的问候("您好~"之类)自然引入(只一句，随后立刻进入正题)。
- 绝对不要聊天气、周末、吃饭、健康、家人、爱好之类的私人闲扯，一句都不行。
- 不要自行添加 beat 里没有的话题(闲谈)。除开场一句问候外，每条消息只谈 beat 指定的业务内容。
...
这是业务对话 — 围绕发货/库存/订单/结算/采购等正事。开场可以用一句轻问候("您好~")自然引入, 但不要多余闲聊。
供货方向固定: 韩国方供货/发货, 越南方永远是收货(被供货)一方 — 绝不可反过来。
...
대화는 "안녕하세요~" 같은 가벼운 인사 한 마디로 부드럽게 시작해도 됩니다(딱 한 줄, 이후 바로 본론).
공급 방향 고정: 한국 측이 공급/발송, 베트남 측은 항상 공급받는(수령) 쪽 — 절대 반대로 쓰지 마세요.

...
_SYSTEM = (
    ...
    "- '홍보 방향'은 진행 제품과 타깃 시장에 맞춰 2~3개의 전략 방향을 번호로 제시하고 "
    "각 방향에 간단한 실행 포인트를 덧붙인다(보고서 톤: 간결한 개조식).\n"
    "- '진행 이슈사항'은 데이터상 드러나는 이슈(예: 무응답/미배포/저조한 지표)와 "
    "대응책을 1~2개로 제시한다. 명확한 이슈가 없으면 '특이 이슈 없음'으로 적는다.\n"
    "- 출력은 정확히 아래 JSON 형식만: "

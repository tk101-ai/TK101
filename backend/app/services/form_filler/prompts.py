...
6. 자료 우선순위: 사용자 업로드 자료(kind=user_upload)와 직접입력(kind=user_input)을 1순위로 신뢰하세요. NAS 자료(kind=nas_file)와 웹검색(kind=web_search)은 보조 참고용입니다. 같은 변수에 대해 값이 충돌하면 항상 업로드/직접입력 값을 우선 채택하고, 업로드/직접입력에 답이 있으면 NAS/웹검색으로 덮어쓰지 마세요.

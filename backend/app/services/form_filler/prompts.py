...
[변수 감지 우선순위]
1. 명시 변수 패턴: {{변수명}}, ${변수명}, [변수명]
2. 라벨 + 빈 칸: "보고일자: ___", "담당자 :   "
3. 표의 헤더 셀에 인접한 빈 데이터 셀 → 헤더를 변수명으로 추정
4. 체크박스: ☐, "□ 검토 완료"
5. 밑줄: _______ (5자 이상 연속)

[type 분류 규칙]
- text: 자유 텍스트 (대부분)
- number: 단위가 명확하거나 숫자만 들어가는 칸 ("금액", "수량")
- date: "일자", "날짜", "기간" 라벨
- enum: 선택지 명시된 칸 (예: "구분 (□신규 □변경)")
- checkbox: 단일 체크박스
- table_row: 표가 N개 행을 동적으로 가질 때 (예: "품목별 수량")
- image: 이미지 자리 표시자

[제약]
- 같은 변수가 여러 번 등장하면 1개로 통합 (key는 첫 등장 위치 기준)
- key는 반드시 영문 snake_case 또는 한글 (LLM이 결정)
  ...
  "variables": [
    {
      ...
      "type": "text" | "number" | "date" | "enum" | "checkbox" | "table_row" | "image",

...
인증:
- Bearer 토큰은 ``token_manager.get_token()`` 에서 받음. 두 모드 지원:
    (a) 수동: ``TENCENT_AIGC_API_KEY`` env 직접 주입 (fallback).
    (b) 자동: ``TENCENT_AIGC_SECRET_ID/KEY/SUBAPP_ID`` 로 ``CreateAigcApiToken``
        호출 후 ApiToken 자동 발급/캐시 (55분 TTL).
- 호출 시점마다 토큰을 받아오므로 만료/회전 처리 자동.
...
키/시크릿 모두 미설정 시 ``RuntimeError``. 라우터가 503 으로 응답하게 함.
"""
...
async def _build_async_client() -> Any:
    """AsyncOpenAI 클라이언트 빌드. 토큰은 매 호출마다 새로 발급/캐시 조회.

    수동 키(``tencent_aigc_api_key``) 미설정 + SecretId/Key 도 미설정이면 RuntimeError.
    """
    try:
        ...
        raise RuntimeError(
            f"텐센트 ApiToken 발급 실패: {exc}"
        ) from exc

    if not api_token:
        raise RuntimeError(
            "텐센트 ApiToken 가용하지 않음 (수동 키도 시크릿도 미설정)."
        )
...
async def stream_chat(
    *,
    ...
    Yields:
        dict: 위 docstring 참고.
    """
    client = await _build_async_client()
